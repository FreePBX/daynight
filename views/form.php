...
if (empty($ids) && ($itemid == '')) {
	...
	<div class="alert alert-danger" role="alert">
		<?php echo _('You have reached the maximum limit for flow controls. Delete one to add a new one') ?>
	</div>
	<?php
	return;
}
...
if (function_exists('recordings_list')) {
	...
	$daydefault   = ($day_recording_id ?? '');
	$nightdefault = ($night_recording_id ?? '');
...
<form name="prompt" id="prompt" class="fpbx-submit" action="?display=daynight" method="post"
	...
	<div class="element-container">
					...
					<div class="form-group">
						<div class="col-md-3">
							<label class="control-label" for="itemid">
								<?php echo _("Call Flow Toggle Feature Code Index") ?>
							</label>
		...
		<div class="row">
			<div class="col-md-12">
				<span id="itemid-help" class="help-block fpbx-help-block">
					<?php echo sprintf(_("There are a total of %s Feature code objects, %s, each can control a call flow and be toggled using the call flow toggle feature code plus the index."), '100', '0-99') ?>
				</span>
			</div>
		</div>
	</div>
	...
	<div class="element-container">
		<div class="row">
			<div class="col-md-12">
				<div class="row">
					<div class="form-group">
						<div class="col-md-3">
							<label class="control-label" for="fc_description">
						...
						<div class="col-md-9">
							<input type="text" class="form-control" id="fc_description" name="fc_description"
								value="<?php echo !empty($fc_description) ? $fc_description : "" ?>">
						</div>
					</div>
				</div>
			</div>
		...
		<div class="row">
			<div class="col-md-12">
				<span id="fc_description-help" class="help-block fpbx-help-block">
					<?php echo _("Description for this Call Flow Toggle Control") ?>
				</span>
			</div>
		</div>
	</div>
	...
	<div class="element-container">
		...
		<div class="row">
			<div class="col-md-12">
				<span id="state-help" class="help-block fpbx-help-block">
					<?php echo _("This will change the current state for this Call Flow Toggle Control, or set the initial state when creating a new one.") ?>
				</span>
			</div>
		</div>
	</div>
					...
					<div class="form-group">
						<div class="col-md-3">
							<label class="control-label" for="password">
								<?php echo _("Optional Password") ?>
							</label>
							<i class="fa fa-question-circle fpbx-help-icon" data-for="password"></i>
						</div>
						<div class="col-md-9">
							<div class="input-group">
								<input type="password" class="form-control clicktoedit" id="password" name="password"
									value="<?php echo !empty($password) ? $password : "" ?>">
								<span class="input-group-addon toggle-password" id="pwtoggle" data-id="password"><i
		...
		<div class="row">
			<div class="col-md-12">
				<span id="password-help" class="help-block fpbx-help-block">
					<?php echo _("You can optionally include a password to authenticate before toggling the call flow. If left blank anyone can use the feature code and it will be un-protected") ?>
				</span>
			</div>
		</div>
	</div>
					...
					<div class="form-group">
						<div class="col-md-3">
							<label class="control-label" for="goto0">
								<?php echo _("Normal Flow (Green/BLF off)") ?>
							</label>
							<i class="fa fa-question-circle fpbx-help-icon" data-for="goto0"></i>
						</div>
						<div class="col-md-9">
							<?php echo drawselects(($dests['day'] ?? ''), 0); ?>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="row">
			<div class="col-md-12">
				<span id="goto0-help" class="help-block fpbx-help-block">
					<?php echo _("Destination to use when set to Normal Flow (Green/BLF off) mode") ?>
				</span>
			</div>
		</div>
	</div>
	...
	<div class="element-container">
		<div class="row">
			<div class="col-md-12">
				<div class="row">
					<div class="form-group">
						<div class="col-md-3">
							<label class="control-label" for="goto1">
								<?php echo _("Override Flow (Red/BLF on)") ?>
							</label>
							<i class="fa fa-question-circle fpbx-help-icon" data-for="goto1"></i>
						</div>
						<div class="col-md-9">
							<?php echo drawselects(($dests['night'] ?? ''), 1); ?>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="row">
			<div class="col-md-12">
				<span id="goto1-help" class="help-block fpbx-help-block">
					<?php echo _("Destination to use when set to Override Flow (Red/BLF on) mode") ?>
				</span>
			</div>
		</div>
	</div>
